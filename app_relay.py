...
def handle_request_relay(data):
    ...
    logger.info(f"房间 {room_id} 启用中继模式 (视频帧转发)")


@socketio.on('relay_video_frame')
def handle_relay_video_frame(data):
    """中继模式：接收客户端视频帧并转发给房间内其他用户"""
    from flask import request
    room_id = data.get('room_id')
    frame_data = data.get('frame')  # base64 编码的 JPEG 图像
    sid = request.sid
